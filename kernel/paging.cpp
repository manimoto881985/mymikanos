...
void SetupIdentityPageTable() {
  ...
  ResetCR3();
  SetCR0(GetCR0() & 0xfffeffff); // Clean WP
}
...
WithError<size_t> SetupPageMap(
  ...
  while (num_4pages > 0) {
    ...
    if (page_map_level == 1) {
      ...
      auto [ num_remain_pages, err ] =
        SetupPageMap(child_map, page_map_level - 1, addr, num_4pages, writable);
      if (err) {
        return { num_4pages, err };
      }
      num_4pages = num_remain_pages;
    }
  ...
  return { num_4pages, MAKE_ERROR(Error::kSuccess) };
}
...
Error SetupPageMaps(LinearAddress4Level addr, size_t num_4pages, bool writable) {
  auto pml4_table = reinterpret_cast<PageMapEntry*>(GetCR3());
  return SetupPageMap(pml4_table, 4, addr, num_4pages, writable).error;
}
